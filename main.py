...
from decoyable.scanners import deps, secrets, sast
    ...
    try:
        ...
        if scan_type in ('sast', 'all'):
            ...
            if vulnerabilities:
                ...
                for vuln in sorted(vulnerabilities, key=lambda x: severity_order.index(x["severity"].value if hasattr(x["severity"], 'value') else str(x["severity"]))):
                    print(f"[{vuln['severity'].value if hasattr(vuln['severity'], 'value') else vuln['severity']}] {vuln['vulnerability_type'].value if hasattr(vuln['vulnerability_type'], 'value') else vuln['vulnerability_type']} - {vuln['file_path']}:{vuln['line_number']}")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    scan.add_argument("scan_type", choices=["secrets", "deps", "sast", "all"], 
                     help="Type of scan to perform")
    scan.add_argument("path", nargs="?", default=".", 
                     help="Path to scan (default: current directory)")
    scan.add_argument("--format", choices=["text", "verbose"], default="text",
                     help="Output format")
